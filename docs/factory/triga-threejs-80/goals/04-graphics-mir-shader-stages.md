...
depth/stencil with stencil masks, vertex count), multi-target fragment
output emission, and resource count surfacing. HIR inventory landed
(`a5c24aa1f`): `is_vertex`/`is_fragment` flags exist on `HirFunction`.
Remaining: device_role Vertex/Fragment mapping (graphics path still bypasses
MIR lowering), and combined pipeline-layout descriptors with stage visibility.
...
- Remaining Stage 4 gate items: device_role Vertex/Fragment mapping — HIR
  `is_vertex`/`is_fragment` flags exist (`a5c24aa1f`) but the graphics path
  still bypasses MIR lowering; `MirDeviceRole::Vertex`/`Fragment` variants
  are the next unit. Combined pipeline-layout descriptors with stage
  visibility (resource counts surfaced via `vertex_resource_count` /
  `fragment_resource_count` (`e55263119`); full bindings remain on per-stage
  `launch_plan()`).

## Gate Check (2026-07-18)

Stage 4 is **not complete**. The gate is not met because source-level MIR
lowering from `@vertex`/`@fragment` annotations does not exist — HIR
`is_vertex`/`is_fragment` flags landed (`a5c24aa1f`) but the graphics path
still bypasses MIR. All reflection and WGSL emission is test-constructed,
not lowered from Faber source. The reflection and WGSL emission subset is
closed; device_role mapping and full resource integration remain.

| Gate item | Status | Evidence |
| --- | --- | --- |
| Faber fixture lowers to typed vertex + fragment MIR and valid WGSL | **OPEN** | HIR `is_vertex`/`is_fragment` flags landed (`a5c24aa1f`); device_role mapping is next unit. Reflection is still test-constructed; no MIR lowering from source. WGSL validates with naga when available. |
...
- Source-level MIR lowering via device_role Vertex/Fragment mapping (Radix —
  HIR flags `a5c24aa1f` landed; `MirDeviceRole::Vertex`/`Fragment` next unit;
  blocks gate item 1)
