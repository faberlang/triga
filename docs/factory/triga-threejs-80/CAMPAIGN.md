...
**Status**: active (2026-07-14) — Stage 1 **foundation CLEARED**; Stage 2 generated-Rust scene identity acceptance and direct scene-store Radix check **green**
...
**Selected next stage**: Stage 4 — graphics MIR and shader-stage handoff; keep the Stage 2 scene-store direct Radix check in Triga's compile gate (readiness map: [`stage4-readiness-map-2026-07-14.md`](stage4-readiness-map-2026-07-14.md))
...
| --- | --- | --- |
| Triga public API | Vector3/Matrix4 + extended families (quat, Euler fail-closed, color, Box/Sphere/Plane/Ray) on main `c3f2972`; **scene store** generated-Rust acceptance and direct Radix check green after reparent fixture correction | Keep transform + scene source gates green |
...
preserving siblings. The direct Radix check for `exempla/triga-scene-store.fab`
is now part of `scripta/check-compile`.
