...
| --- | --- | --- |
| Triga math and transforms | Camera yaw/pitch fact records, ray, AABB, transform payload, packaged collision extents, first-person planar movement, face-code fact records, face-code unit quads, and indexed-cube view-projection facts are locked | Preserve and consume |
...
records, packaged `Box3` overlap facts, and normalized planar movement deltas.
...
mounts and mount failures, and exposes reverse-order disposal for returned
cleanup values. The `examples/browser-app` harness now consumes that generated
lifecycle helper. HV-03C now has a Hello Voxel scaffold automation slice:
...
`mountControllers().dispose()`. HV-03E now has an executable resize slice:
`examples/browser-app` defines a `resize_controller` that returns its
`dom.on_resize` subscription; the Node DOM harness proves initial and
dispatched resize delivery update visible state, and generated disposal removes
the resize listener. Goal 03 still needs source-authored keyboard, pointer,
focus, pointer-lock, and typed browser failure coverage.
