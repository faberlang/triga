...
| Browser application runtime | `faber-web` contracts and Faber `browser-app` packaging exist; `examples/hello-voxel` now has a package-admission scaffold; frame, resize, keyboard, pointer, focus, and pointer-lock subscription contracts exist in `web:dom`; generated browser entries now expose controller mount/failure/dispose lifecycle records, and the `examples/browser-app` harness consumes that generated helper | Extend through Goal 03 for frame-owned application runtime and Hello Voxel consumer automation |
...
lifecycle helper. HV-03B still needs frame-owned application runtime coverage,
and HV-03C remains pending for Hello Voxel-specific consumer automation.
