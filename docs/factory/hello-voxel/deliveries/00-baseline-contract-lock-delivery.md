...
lifecycle handle facts, visible mesh draw packets, face-code metadata,
visible-face mesh fact records, colored quad finalization, and colored mesh
fact derivation.
...
- `./scripta/check-hello-voxel-contract` from `triga` for the complete
  Triga-owned source, exemplar, compile, capability, and runtime-dependency
  inventory gate.
