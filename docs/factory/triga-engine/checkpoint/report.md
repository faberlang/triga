...
| Nested-package tooling (DEFER-121) — **RESOLVED, four gaps (2026-08-01)**: G1 faber TS emitter never emits nested library modules (`faber/src/package/product.rs` ~1467/~1822 — two non-recursive `read_dir`; **FIXED** in faber commit `34a826d`, corpus builds green); **G1b hollow-facade namespace imports** — radix TS emitter `write_module_exports` early-returns on empty exports (map-only facade emits `export {}` only) but importers always emit `import { alias }` (TS2305); hit when `triga.fab` imported flat `triga:material` post-split (fixed triga-side by leaf imports; the emitter lane should handle hollow-facade namespace imports); G2 enum variants are module-scoped — no cross-module construction/matching of `discretio` variants (`finge`→SEM001, `casu`→SEM001, named variant import→`name_not_variant`, qualified→`SEM002`; blocks scene split outright, forced `PrimitiveTopology` into `geometry/data` in DS-E); G3 faber rejects library import cycles (`PKG001`) — scene node↔store↔query is genuinely cyclic | G1 fixed (faber `34a826d`); G1b: radix TS-emitter lane; G2+G3: **language-decision items** (handoff: `language-lane-handoff.md`) | DS-D parked on G2+G3 |
| World-scale target for S6 (culling/LOD/streaming meaningfulness) | open; freeze at S5 with measurements | S5/S6 |
| Proba runner (G4) — MIR stepper cannot lower receiver-method calls: `faber test .` → `unsupported MIR lowering: method call before runtime/provider MIR lowering` / `projection base that does not resolve to a local value`; all 3 of `src/math.proba`'s tests fail (0 passed / 3 failed). The `.proba` layer is non-functional for method-heavy APIs (all Triga tests use receiver methods). Repro: `faber test src/math.proba` (single-file: `SEM002` on the `./math` relative import) and `faber test .` (package: stepper lowering failures) | radix MIR-stepper lane | tracked in `compiler-lane-ledger.md` |
