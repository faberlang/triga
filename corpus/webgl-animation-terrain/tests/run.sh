...
if [[ -z "${FABER_BIN:-}" ]]; then
  echo "triga-corpus-animation-terrain: no faber binary found" >&2
  exit 1
fi
...
cp "$APP_DIR/assets/terrain.wgsl" "$APP_DIR/public/triga-lit.wgsl"
cp "$APP_DIR/assets/terrain.wgsl" "$APP_DIR/src/shaders/test-data/kernel.wgsl"
...
grep -q '"selector": "#triga-corpus-animation-terrain"' "$APP_DIR/dist/controllers.json"
...
grep -q 'fn triga_terrain_vertex' "$APP_DIR/dist/public/triga-lit.wgsl"
...
if (vertex.entry_name !== 'triga_terrain_vertex' || fragment.entry_name !== 'triga_terrain_fragment') process.exit(1);
...
const shader = fs.readFileSync('$APP_DIR/assets/terrain.wgsl', 'utf8');
const checks = [
  main.includes('dom.on_frame'),
  main.includes('data-animation-seconds'),
  main.includes('data-terrain-features'),
  page.includes('mountControllers'),
  shader.includes('fn fbm'),
  shader.includes('fn ridge_fbm'),
  shader.includes('smoothstep'),
  shader.includes('valley_fog'),
];
if (!checks.every(Boolean)) process.exit(1);
"

echo "triga-corpus-animation-terrain checks ok"
